...
export default function WordLab({ lesson, userId, onSaved }: { lesson: Lesson; userId?: string; onSaved: (attempt: PracticeAttempt) => void }) {
  ...
  function targetClass(target: string) { return guided && current?.target === target ? 'guided-target' : ''; }
  const action = (target: string, label: string, className = '') => <button disabled={correct} className={`${className} ${targetClass(target)}`} onClick={() => choose(target)}>{label}</button>;
  ...
  return <section className="lesson-workspace" aria-label={lesson.title}>
    ...
    {phase === 'read' && <div className="lesson-intro"><p className="lesson-kicker">ANTES DE HACERLO</p><p>{lesson.concept}</p><div className="learning-goals"><strong>Al terminar podrás:</strong><ul>{lesson.steps.map(item => <li key={item.target}>{item.goal}</li>)}</ul></div><p className="note">Simulación educativa simplificada de Word para Windows. Algunas posiciones cambian según la versión. No necesitas tener Word instalado para esta práctica.</p><button className="solid-button" onClick={() => begin('guided')}>Comenzar práctica guiada →</button></div>}

    {active && <><div className="mission"><div><span>{guided ? 'GUÍA' : 'RETO'} · PASO {step + 1} DE {lesson.steps.length}</span><h3>{current.goal}</h3>{guided && <p>{current.hint}</p>}</div><span className="mission-count">{step + 1}/{lesson.steps.length}</span></div>
      <div className="simulation-scroll"><div className="simulation-frame">
        <div className="sim-caption"><span>COMPUTADORA DE PRÁCTICA</span><span>No modifica tu equipo</span></div>
        {lesson.id === 'word-start' && <div className="desktop-sim">
          <div className="desktop-content">{step === 0 ? <div className="desktop-shortcuts">{action('documents', 'Carpeta Documentos', 'desktop-shortcut')}{action('browser', 'Navegador', 'desktop-shortcut')}<p>Tu escritorio de práctica</p></div> : <div className="start-menu"><h4>Inicio</h4><form onSubmit={event => { event.preventDefault(); choose(search.trim().toLowerCase() === 'word' ? 'search-word' : 'search-other'); }}><label htmlFor="word-search">Buscar una aplicación</label><div className="search-row"><input id="word-search" autoComplete="off" value={search} disabled={correct || step !== 1} onChange={event => setSearch(event.target.value)} placeholder="Escribe el nombre…"/><button disabled={correct || step !== 1} className={targetClass('search-word')}>Buscar</button></div></form>{step === 2 && <div className="search-results"><p>Aplicaciones</p>{action('word', 'W · Word', 'app-result')}{action('excel', 'X · Excel', 'app-result')}{action('browser', 'Navegador', 'app-result')}</div>}</div>}</div>
        ...
        {lesson.id === 'word-interface' && <div className="word-window">
          ...
          <div className="document-canvas"><button disabled={correct} className={`word-page ${targetClass('page')}`} onClick={() => choose('page')}><span>Mi primer documento</span><p>Estoy aprendiendo a orientarme en Word.</p><p>Aquí escribiré mis ideas y daré forma a mis trabajos.</p><span className="text-caret" aria-hidden="true">|</span></button></div>
          <div className="word-bottom">{action('status','Página 1 de 1 · 23 palabras', 'word-status')}{action('zoom','− ━━━●━━ +   100%', 'word-zoom')}</div>
        </div>}
      </div></div>
      <div className={`lesson-feedback ${correct ? 'correct' : ''}`} aria-live="polite"><p>{feedback || (guided ? 'La zona resaltada te orienta. Realiza la acción indicada.' : 'Realiza la acción. Cuenta la primera elección de cada paso.')}</p>{correct && <button className="solid-button" onClick={next}>{step === lesson.steps.length - 1 ? 'Terminar' : 'Siguiente paso'} →</button>}</div>
    </>}
